  ...
  return (
    <div className="flex flex-col rounded-lg border border-gray-800 bg-gray-950">
      <div className="flex items-center justify-between border-b border-gray-800 px-4 py-3">
        ...
        <button type="button" onClick={closeStats} className="text-gray-500 hover:text-white">
          ✕
        </button>
      </div>
        ...
        <div className="space-y-2">
          {MOCK_STATS.map((stat) => (
            <div key={stat.label} className="grid grid-cols-[1fr_auto_auto] items-center gap-2 text-sm">
              ...
              <span className="w-14 text-center font-semibold text-white">{stat.away}</span>
              <span className="w-14 text-center font-semibold text-white">{stat.home}</span>
            </div>
          ))}
        </div>
      </div>
